...
GitHub: DragonQuix/ai-berkshire（fork 发布远端）；xbtlin/ai-berkshire 为上游参考仓库
...
- 本地克隆路径：`~/ai-berkshire/`
- 远程仓库：`https://github.com/DragonQuix/ai-berkshire.git`
- 推送前先 `git pull --rebase origin main`（远程经常有新提交）
- commit message 用中文，描述清楚改了什么
- 不要推送中间过程文件（如 data_collection.md），只推最终报告
- **行尾卫生（防 `git diff --check` 误报）**：本项目 `core.autocrlf=true` 且无 `.gitattributes`，历史 `.md` 文件多为 CRLF。在 CRLF 文件中插入新行时，`git diff --check` 会把 `\r` 当作 trailing whitespace 报告，CI 启用该项即失败。三条要点：
  - **诊断**：报错指向某行 trailing whitespace 但 `od -c`/`xxd` 看不到空格、且整文件 `count(b'\r\n')` > 0，即为 CRLF 误报，非内容有空白
  - **预检**：写完文档提交前跑 `git diff --check` 与 `git diff --check HEAD~N..HEAD`，确认无误再提交
  - **修复**：用 Python 脚本以二进制方式整文件归一化（`text.replace("\r\n","\n")` 后 `write_bytes`），勿逐行手改；若问题已进提交，对最近提交用 `git commit --amend`（确认远未推送）重写 blob。幂等起见，新增 `.md` 文件优先写 LF
