...
>
> 2026-07-01 继续推进第六十切片：`trim_to_target.reason` 与 `add_to_target.reason` 补齐当前仓位与建议仓位；目标偏离机械建议从泛化“高于/低于目标仓位或上下限”，改为展示“当前 X%，建议回到/补足到 Y%”，避免再平衡表给出目标调仓动作但不说明调仓幅度。`/portfolio-review` 同步要求目标偏离理由展示当前仓位与建议仓位，Codex 工具副本与 skill reference 同步，并用回归测试锁定。
>
> 2026-07-01 继续推进第六十一切片：`reduce_or_exit.reason` 与低估低预期复核理由补齐机会成本触发幅度；低于现金门槛的机械建议从泛化“低于现金门槛”，改为展示“风险调整后 X% 低于现金门槛 Y%”，避免再平衡表建议减仓或复核估值张力时不给收益差距。`/portfolio-review` 同步要求机会成本理由展示风险调整后收益与现金门槛，Codex 工具副本与 skill reference 同步，并用回归测试锁定。
>
> 2026-07-01 继续推进第六十二切片：`review_valuation_tension.reason` 引用 `valuation_sanity.message`，让高估高预期复核建议直接展示 PE 历史分位与预期年化，例如“PE 处于历史 92% 分位与 25% 预期年化存在张力”；现金偏高但候选全有高估张力时也展示同一证据，避免只写“存在张力”但不给估值与收益假设。`/portfolio-review` 同步要求估值水位张力理由展示 PE 分位与预期收益，Codex 工具副本与 skill reference 同步，并用回归测试锁定。
>
> 2026-07-01 继续推进第六十三切片：`deploy_cash_review.reason` 补齐现金部署候选与机会成本依据；现金偏高且存在无高估张力的可部署候选时，理由从泛化“风险调整后收益最高”改为展示“候选名称 + 风险调整后收益”，例如“阿里巴巴 风险调整后 12.0%”，避免再平衡表给出现金用途但不给候选收益证据。`/portfolio-review` 同步要求现金部署理由展示候选名称与风险调整后收益，Codex 工具副本与 skill reference 同步，并用回归测试锁定。

- 持仓组合健康度评估
- 行业/地域集中度分析
- 相关性风险检测
- 机会成本（含负预期收益、字段级输入错误、缺失输入建议与数据不足展示，机会成本理由展示风险调整后收益与现金门槛，并纳入整体健康度降级/数据不足判断）、压力测试、目标仓位偏离、目标调仓测算、目标覆盖度、机械再平衡建议（含目标偏离理由展示当前仓位与建议仓位、低配缺输入冲突消解、高估高预期加仓软门禁、低估低预期减仓软门禁、估值水位张力理由展示 PE 分位与预期收益、集中度理由展示第一大持仓占比与上限、现金缓冲理由展示当前占比与阈值、现金部署理由展示候选名称与风险调整后收益、单一暴露首要动作与动作口径语义化、按风险等级与暴露权重选择复核对象、复核暴露优先级跟随风险等级、复核暴露理由展示具体比例）、首屏结论摘要、动作口径与数据缺口、可配置现金门槛、持仓名称/代码唯一性校验、估值水位张力校验（PE 分位与预期收益的张力，含不替代人工裁决口径）
