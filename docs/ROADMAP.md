...
- P2-F4：`report_audit extract` 情景标题误抽与多列表格列对齐错位。=> 候选，待真实样本固化。
...
- 2026-07-01：完成新华保险 deep 档第三次运行反馈 P1-F1/F2/F3，`report_audit extract` 支持 `-o/--output` 落盘，口径列检查跳过压力测试/偏误自查等非核心表，保险股财务口径统一为 `oi=营业收入`；P2-F4/F5 与 P3-F6/F7 保留为候选。
