...
- P1-B3：财务口径元数据，把理杏仁字段口径、年报口径、币种和单位传到报告与审计环节。=> 已完成。
- P1-C1：离线 fixture 与 GitHub Actions CI，把 P1 反馈修复转为长期回归门禁。=> 已完成。
- P1-D1：release notes 自动生成，用本地 Git 提交生成可审阅的 Markdown 发布说明。=> 已完成。
- P1-E1：`report_audit` 单位归一化，避免报告值与核验值单位错配造成 9900% 假偏差。=> 已完成。
- P1-E2：`report_audit verdict` 可复现输出，支持 `-o/--output` 直接落盘纯 JSON 判决结果。=> 已完成。
- P1-E3：`report_audit verdict` 口径认可通道，支持 `caliber_ack` + `caliber_note` 将已解释口径差异从警告/失败中单列。=> 已完成。
- P2-E4：港股 `industry-compare --no-mx` 路径透出 `note` 与 `alternatives`，避免文档省额度路径丢失降级提示。=> 已完成。
- P2-E5：`report_audit` 口径列覆盖率检查，标记缺少「口径/来源」列的核心数据表并在 verdict 输出元数据警告。=> 已完成。
- P2-E6：港股年报 `reportType` 兜底，按 `reportType=annual_report` 识别港股年报，避免 fiscal year 非 12-31 时年报序列为空。=> 已完成。
- P3-E7：Agent 降级错误摘要脱敏标准，明确可保留模型名/错误码/HTTP 状态，必须删除路径、token、cookie、账户名等敏感信息。=> 已完成。
- P3-E8：`docs/dev-feedback-action-plan.md` 二次验证跟踪字段，区分自动回归与真实技能运行二次验证，未验证项标记待补。=> 已完成。
- P1-F1：`report_audit extract` 可复现落盘，支持 `-o/--output` 直接写出纯 JSON 抽检清单。=> 已完成。
- P1-F2：`report_audit` 口径列检查降噪，避免压力测试、偏误自查、情景表等非核心表误报缺口径列。=> 已完成。
- P1-F3：保险股 `oi/toi` 口径对齐，工具元数据与 `/investment-research` 文档统一说明保险股用 `oi=营业收入`。=> 已完成。
- P2-F4：`report_audit extract` 情景标题误抽与多列表格列对齐错位。=> 已完成。
- P2-F5：Agent 路由间歇性失败的文档诚实性与每次派发独立捕获说明。=> 已完成。
- P3-F6：deep 档抽检点数量下限自适应。=> 已完成。
- P3-F7：datapack 顶层 `_generated_at` 与 TTL 可观测。=> 已完成。
- P1-F8：港股油气/贸易类 `toi/npatoshopc` 口径提示，避免直接按理杏仁 `toi`/`npatoshopc` 计算 EPS/PE 时忽略贸易业务全额或权益法份额差异。=> 已完成。
- P1-F9：`report_audit extract` 口径列内嵌数字误匹配，跳过 `口径/来源` 列中的分位、年份、同比等说明数字。=> 已完成。
- P1-F10：`report_audit` 定性评级表口径列误报降噪，避免护城河/决策/信心度评分表淹没真正缺口径列的核心财务表。=> 已完成。
- P2-F11：`/investment-research` datapack TTL 使用说明，读取 `_generated_at`/`_ttl_seconds`/`_expires_at` 并在过期时重新拉取。=> 已完成。
- P2-F12：`/investment-research` 长报告 heredoc 防错说明，分段写入先写临时文件再追加正式报告。=> 已完成。
- P3-F13：`/investment-research` 编辑冲突处理说明，遇 `File has been modified since read` 先重新读取目标行区域。=> 已完成。
- P2-F14：周期股“周期顶部 PE 陷阱”检测器，覆盖油气/煤炭/钢铁/航运/券商并接入 `/investment-research` 估值节。=> 已完成。
...
- 2026-07-01：完成 P1-B3 财务口径元数据，`lxr_data.py financials/datapack` 输出 `caliber_metadata`，保留 `toi=营业总收入`、年报“收益”口径提醒、币种与原始单位；`cross-validate --caliber` 在偏差时提示「口径待核对」，`investment-research` 要求核心数据表包含「口径/来源 / 币种 / 单位」，root 工具、Codex bundled 副本和 skill reference 已同步。
- 2026-07-01：完成 P1-C1 离线 fixture 与 GitHub Actions CI，新增 `.github/workflows/ci.yml`，在 PR/push 上运行全量 pytest、`verify_channel_capability --quick`、compileall、release smoke 和 `git diff --check`；新增 `tests/fixtures/investment_research_smoke/`，固化 datapack 口径元数据、Task Agent 降级记录和抽检报告骨架合同。
- 2026-07-01：完成 P1-D1 release notes 自动生成，新增 `tools/release_notes.py`，默认按 `origin/main..HEAD` 读取本地提交并按 conventional commit 类型生成中文 Markdown；`tools/release_smoke.py` 增加 release notes dry run，`docs/release-notes.md` 记录发布前草稿生成流程。
- 2026-07-01：完成 P1-E1 `report_audit` 单位归一化，verdict 支持 `report_unit`/`fetched_unit` 与常见数量单位换算（如 `1430 百万` -> `14.30 亿`），缺少 `fetched_unit` 且量级差超过 50 倍时降为 warning 并提示补单位；extract JSON 模板新增 `report_unit`、`fetched_unit`、`fetched_unit2`，root 工具与 Codex bundled 副本已同步。
- 2026-07-01：完成 P1-E2 `report_audit verdict` 可复现输出，CLI 新增 `-o/--output` 将判决结果写入 UTF-8/LF JSON 文件，stdout 保留人类可读准出/打回摘要，stderr 写落盘提示；root 工具与 Codex bundled 副本已同步。
- 2026-07-01：完成 P1-E3 `report_audit verdict` 口径认可通道，results JSON 支持 `caliber_ack: true` + 非空 `caliber_note`，将已在报告中解释的超阈值口径差异计入 `caliber_ack_items`，摘要区分「通过/口径认可/警告/不通过」；root 工具、Codex bundled 副本和 `investment-research` 文档已同步。
- 2026-07-01：完成 P2-E4 港股 `industry-compare --no-mx` 路径降级提示，CLI 接受 `--no-mx` 兼容省额度调用，港股仍返回 `note: 申万行业分类仅覆盖A股` 与静态 `alternatives`；root 工具、Codex bundled 副本和 `investment-research` 文档已同步。
- 2026-07-01：完成 P2-E5 `report_audit` 口径列覆盖率检查，extract 会对含 PE/PB/营收/归母净利润/市值/现金流等核心字段但缺少「口径/来源」表头的 Markdown 表格写入 `caliber_column_warning`，verdict 单列 `metadata_warn_items`，不阻塞准出但提示回补表头；root 工具、Codex bundled 副本和 `investment-research` 文档已同步。
- 2026-07-01：完成 P2-E6 港股年报 `reportType` 兜底，`_annual_fs_records()` 支持 `reportType=annual_report`，并统一 `verify-inputs`、`industry-deep`、quality/compare 等年报消费路径；root 工具与 Codex bundled 副本已同步。
- 2026-07-01：完成 P3-E7/E8 反馈闭环收敛，10 个团队型 skill 增加 Agent 降级错误摘要脱敏标准，`docs/dev-feedback-action-plan.md` 增加二次验证跟踪字段；root skill 与 Codex reference 已同步。
- 2026-07-01：完成新华保险 deep 档第三次运行反馈 P1-F1/F2/F3，`report_audit extract` 支持 `-o/--output` 落盘，口径列检查跳过压力测试/偏误自查等非核心表，保险股财务口径统一为 `oi=营业收入`；P3-F7 保留为候选。
- 2026-07-01：完成 P2-F4 `report_audit extract` 解析降噪，非可审计压力/偏误表整表跳过，Markdown 表格数据行列数不匹配时直接跳过，不再生成 `列N` 猜测标签；root 工具与 Codex bundled 副本已同步。
- 2026-07-01：完成 P2-F5 Agent 路由间歇性说明，10 个团队型 skill 明确路由失败可能间歇性发生，每次 TeamCreate/TaskCreate/后台 Agent 派发、等待或读取结果都必须独立捕获，不得因前一次成功假设后续成功；同一路由失败后不反复重试，直接降级为顺序角色模拟。
- 2026-07-01：完成 P3-F6 deep 档抽检数量自适应，`report_audit extract --depth deep` 默认 25% 抽样、至少 30 点且不再 30 点封顶；`/investment-research` 文档已要求 deep 档使用该参数，root 工具/skill 与 Codex 副本已同步。
- 2026-07-01：完成 P3-F7 datapack 顶层 `_generated_at` 与 TTL 可观测，`get_research_datapack` 输出 `_generated_at`/`_ttl_seconds`/`_expires_at`，缓存命中时保留原始生成时间而非命中时间；root 工具与 Codex bundled 副本已同步，新增两条回归测试。
- 2026-07-02：完成中国海油港股 deep 档第四次运行反馈 P1-F8/F9/F10，港股非金融口径元数据补充油气/贸易/权益法提示，`report_audit extract` 跳过口径列内嵌数字，并对无财务单位的定性评级表降噪；root 工具与 Codex bundled 副本已同步。
- 2026-07-02：完成中国海油港股 deep 档第四次运行反馈 P2-F11/F12/P3-F13，`/investment-research` 补充 datapack TTL 过期判断、长报告分段写入 heredoc 防错、`File has been modified since read` 行尾归一化冲突处理；root skill 与 Codex reference 已同步。
- 2026-07-02：完成中国海油港股 deep 档第四次运行反馈 P2-F14，新增 `tools/cycle_pe_trap.py` 周期顶部 PE 陷阱检测器，覆盖油气/煤炭/钢铁/航运/券商，Codex bundled 工具同步，`/investment-research` 估值节已接入。
