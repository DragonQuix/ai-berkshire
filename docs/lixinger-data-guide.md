...
| `lhb-detail [code] --date YYYY-MM-DD` / `--trade-id ID` / `--start-date YYYY-MM-DD --end-date YYYY-MM-DD` | A股龙虎榜买卖席位明细；区间模式先筛龙虎榜记录再按 `trade_id` 批量拉详情，并输出 `range_flow_summary` / `range_seat_profile_summary`；可用 `--dominant-type` / `--dominant-direction` 过滤资金主导方，用 `--youzi-alias` 过滤指定游资/活跃席位，或用 `--min-dominant-net` 过滤主导资金绝对净额下限；席位含 `seat_category`、`seat_profile`，记录含 `seat_profile_summary` / `seat_amount_summary` / `seat_flow_analysis`；`range_seat_profile_summary` 基于过滤后 records 按画像类型与别名/席位名聚合买卖次数、金额、净额、交易日和 top seats，并输出 `youzi_alias_strengths` 按游资 alias 聚合买卖次数、金额、净额、方向、交易日和席位名，默认按绝对净额降序；`recognition_summary` 汇总可识别席位数量、游资别名数、可识别净额绝对值、游资净额绝对值、可识别主导类型、区间内席位/净额占比和 `top_youzi_seats`；`source_detail: legacy:ashare_data/lhb-detail` | `lhb-detail 000004 --start-date 2026-06-01 --end-date 2026-06-26 --min-dominant-net 500000` |
| `lhb-compare <code> <code>... --start-date YYYY-MM-DD --end-date YYYY-MM-DD` | A股龙虎榜多股票区间辨识度对比；复用 `lhb-detail` 区间模式与 `--dominant-type` / `--dominant-direction` / `--youzi-alias` / `--min-dominant-net` 过滤，输出 `rows`，每行含可识别净额绝对值、游资净额绝对值、top 游资 alias、方向、排名、`youzi_identity_tag`、共同/独有游资占比和辨识 top alias；默认按 `youzi_abs_net_amount` 降序，可用 `--sort-by profiled_abs_net_amount` 或 `--sort-by profiled_abs_net_ratio` 切换排序；`comparison_summary` 汇总代码数、命中代码数、领先代码、共同游资 alias、alias 覆盖频次、`shared_youzi_strength_summary`（共同 alias 绝对净额、全部 alias 绝对净额、占比和 top 共同 alias）、`shared_youzi_concentration_summary`（共同 alias 总绝对净额、top 共同 alias 占比、覆盖代码数、方向一致性和集中度分层）、`shared_youzi_code_strengths`（每只股票的共同 alias 绝对净额、占比和 top 共同 alias）、`unique_youzi_code_strengths`（每只股票的独有 alias 绝对净额、占比和 top 独有 alias）、`youzi_code_identity_tags`（每只股票标注 `shared_dominant` / `unique_dominant` / `balanced` / `no_youzi`）、`youzi_code_identity_summary`（各标签数量、占比、各标签代表代码、整体主导标签和主导占比）、`youzi_alias_cross_code_strengths`（每个 alias 在各代码上的净额、绝对净额与方向）、`youzi_direction_consistency_summary`（共同 alias 同向/分歧数量、占比和主导一致性）以及 `same_direction_youzi_aliases` / `mixed_direction_youzi_aliases`；`source_detail: legacy:ashare_data/lhb-compare` | `lhb-compare 000004 000005 --start-date 2026-06-01 --end-date 2026-06-26 --sort-by profiled_abs_net_amount` |
| `lhb-industry-compare <code> --start-date YYYY-MM-DD --end-date YYYY-MM-DD` | A股同申万行业龙虎榜辨识度对比；先用理杏仁 `cn/industry/constituents` 找锚定股票所属行业成分，锚定代码排第一，再取最多 `--max-codes` 个代码复用 `lhb-compare`；默认输出对比可用性、命中覆盖、领先差距、组合信号、Top 游资、方向一致性、共同/独有贡献和行级辨识摘要；需要机器读取时加 `--json`；`source_detail: lixinger+legacy:lhb-industry-compare` | `lhb-industry-compare 600519 --start-date 2026-06-01 --end-date 2026-06-26 --max-codes 4` |
