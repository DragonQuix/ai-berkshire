...
| `lhb-detail [code] --date YYYY-MM-DD` / `--trade-id ID` | A股龙虎榜买卖席位明细（东方财富免费源；席位含 `seat_category`、`seat_profile`，记录含 `seat_profile_summary`；`source_detail: legacy:ashare_data/lhb-detail`） | `lhb-detail --trade-id 100357777` |
