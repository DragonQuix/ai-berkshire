...
最终报告写完后，用抽样命令从报告中生成 contract 格式的抽检清单，避免手工把数据点翻译成 `items` 结构：

```bash
python tools/team_research_outputs.py audit-extract reports/{公司名} --ratio 0.15 [--seed N]
```

该命令读取 `最终报告.md`，复用 `tools/report_audit.py` 的数据点抽取与采样逻辑，把每个采样点写入 `audit-results.json.items[*]` 的 contract 结构：`claim` / `report_location`（第 N 行）/ `source_ref`（空，待 Team Lead 补）/ `expected_value` / `verified_value`（空）/ `status=pending` / `note`（原文摘录）。重新抽取会把 `verdict` 重置为 `reject`，必须完成核验后才能改回 `pass`。支持 `--ratio` 调整抽样比例与 `--seed` 复现同一批样本。Team Lead 随后逐项补 `source_ref`、`verified_value` 和 `status`，再交给 `validate` 把关。

准出前用结构校验命令检查必需产物是否齐全：
...
`items[*].status` 只能是 `pass`、`fail` 或 `pending`：`pending` 表示尚未核验（`audit-extract` 草稿态），`pass`/`fail` 表示已从可靠信源取数并比对完成。每个 item 的 `claim`、`report_location`、`expected_value` 必须非空；当 `status` 为 `pass` 或 `fail` 时，`verified_value` 与 `source_ref` 也必须非空，且 `source_ref` 须在 `source-index.md` 的 ref 列中定义。`verdict` 为 `pass` 时，`items` 必须非空且全部 `status == pass`，即一项 `pending`/`fail` 或空清单都不得判决准出。
