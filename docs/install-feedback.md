...
- 平台：Windows PowerShell 7 / `pwsh`、macOS/Linux shell、WSL 或其他兼容层。
- 工具版本：OS、git、python、Claude Code、Codex App。
- 安装命令：完整命令，但必须脱敏本地隐私路径。
- Claude Code 自检：`~/.claude/commands` 下是否有 19 个 AI Berkshire `.md` 命令；如果用户已有其他 Claude Code 命令，总数可以超过 19。
