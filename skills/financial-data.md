...
python tools/lxr_data.py lhb-detail 000004 --start-date 2026-06-01 --end-date 2026-06-26 --quiet  # 区间批量席位明细；含 range_flow_summary / range_seat_profile_summary
...
python tools/lxr_data.py lhb-detail 000004 --start-date 2026-06-01 --end-date 2026-06-26 --youzi-alias 拉萨天团 --quiet
python tools/lxr_data.py lhb-detail 000004 --start-date 2026-06-01 --end-date 2026-06-26 --min-dominant-net 500000 --quiet
python tools/lxr_data.py lhb-compare 000004 000005 --start-date 2026-06-01 --end-date 2026-06-26 --sort-by profiled_abs_net_amount --min-recognition-score 50 --quiet
python tools/lxr_data.py lhb-industry-compare 600519 --start-date 2026-06-01 --end-date 2026-06-26 --max-codes 4 --quiet
...
| A股龙虎榜买卖席位 | `lxr_data.py lhb-detail --trade-id 100357777`、`lhb-detail 000004 --date 2026-06-26` 或 `lhb-detail 000004 --start-date 2026-06-01 --end-date 2026-06-26 --min-dominant-net 500000`（区间模式先筛龙虎榜记录再按 `trade_id` 批量拉详情；区间 payload 含 `range_flow_summary` / `range_seat_profile_summary`，后者基于过滤后 records 按画像类型与别名/席位名聚合次数、金额、净额和 top seats，并在 `youzi_alias_strengths` 中按游资 alias 聚合买卖次数、金额、净额、方向、交易日和席位名，默认按绝对净额降序；`recognition_summary` 汇总可识别席位数量、游资别名数、可识别净额绝对值、游资净额绝对值、可识别主导类型、区间内席位/净额占比和 `top_youzi_seats`；可按主导类型/方向、游资别名或主导资金绝对净额下限过滤；席位含 `seat_category` / `seat_profile`，记录含 `seat_profile_summary` / `seat_amount_summary` / `seat_flow_analysis`，`source_detail: legacy:ashare_data/lhb-detail`） | 东方财富网页明细页 |
| A股龙虎榜多股对比 | `lxr_data.py lhb-compare 000004 000005 --start-date 2026-06-01 --end-date 2026-06-26 --sort-by profiled_abs_net_amount --min-recognition-score 50` 或 `lxr_data.py lhb-industry-compare 600519 --start-date 2026-06-01 --end-date 2026-06-26 --max-codes 4`（复用 `lhb-detail` 区间模式与过滤条件；`lhb-compare` 需显式给定 2-4 个代码，`lhb-industry-compare` 会先用理杏仁申万行业成分取锚定股票同板块代码并复用 `lhb-compare`；输出每只股票的可识别净额绝对值、游资净额绝对值、top 游资 alias、方向、排名、行级 `youzi_identity_tag`、共同/独有占比、辨识 top alias、`youzi_recognition_score` 与 `youzi_recognition_rank_reason`；非 JSON 输出会在榜单前打印对比可用性与命中覆盖，行级打印辨识分和标签；默认按 `youzi_abs_net_amount` 排序，也可按 `profiled_abs_net_amount` / `profiled_abs_net_ratio` / `youzi_recognition_score` 排序，并可用 `--min-recognition-score` 过滤综合辨识分低于阈值的行，同时保留 `filtered_out_codes` 说明被过滤代码；`comparison_summary` 汇总代码数、命中代码数、`code_coverage_summary` 命中覆盖摘要、`compare_readiness_summary` 对比可用性摘要、领先代码、共同游资 alias、alias 覆盖频次、`shared_youzi_strength_summary` 共同游资强度占比、`shared_youzi_concentration_summary` 共同游资集中度、`shared_youzi_code_strengths` 每股共同游资贡献度、`unique_youzi_code_strengths` 每股独有游资贡献度、`youzi_code_identity_tags` 每股游资辨识来源标签、`youzi_code_identity_summary` 标签数量/占比汇总、`youzi_recognition_leaderboard` 综合辨识度排行、`youzi_recognition_gap_summary` 领先代码与第二名分差、`top_youzi_alias_comparison` 每股 Top 游资别名横向对比、`youzi_composite_signal_summary` 共同/独有/方向/集中度组合信号、`youzi_direction_consistency_summary` 方向一致性汇总、`youzi_alias_cross_code_strengths` 跨代码净额强度明细，以及 `same_direction_youzi_aliases` / `mixed_direction_youzi_aliases`；`source_detail: legacy:ashare_data/lhb-compare` 或 `lixinger+legacy:lhb-industry-compare`） | 东方财富网页明细页 |
