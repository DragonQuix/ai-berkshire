...
`weight` 可用百分数或小数，工具会自动归一化。`expected_return` 为预期年化收益，`conviction` 为确定性/置信度，均可用小数或百分数。报告中必须引用 `portfolio_analyzer` 输出的 `overall_health`、`concentration`、`exposures`、`risk_flags`、`correlation_risks`、`stress_tests` 与 `opportunity_cost`。该工具只做组合结构诊断，不替代后续个股估值、新闻核验和最终调仓判断。
