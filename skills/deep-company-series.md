...
> **数据注入**：每篇开写前，按篇目拉取理杏仁数据包（与 `/investment-research` 第〇步相同），精确数字写入正文并标注 `_source: lixinger`。篇目 06/08 必须用 `financial_rigor.py` 验算。

| # | 篇名模板 | 核心问题 | 字数 | 优先数据命令 |
|---|---------|---------|------|-------------|
| 01 | 你以为你看懂了 X，其实没有 | 认知重置：破 3 个常见错觉 | 4,000-5,000 | `revenue` + mx-search |
| 02 | X 的护城河——`<生意本质一句话>` | 护城河深不深、未来 5/10 年还在不在 | 6,000-8,000 | `revenue` + `industry-compare` |
| 03 | X 的最大利润引擎——`<最赚钱业务>` | 主业是什么、为什么能持续 | 6,000-8,000 | `revenue` + `financials` |
| 04 | X 藏在账上的另一家公司——`<隐藏资产>` | 投资组合 / 子公司 / 隐藏价值 | 8,000-10,000 | `financials` + 年报附注 |
| 05 | AI（或当下叙事）时代，X 是赢家还是输家 | 时代变量：分业务拆 AI 影响 | 8,000-10,000 | mx-search + `revenue` |
| 06 | 用巴菲特方式拆 X 的财报 | 财务深度：毛利率/FCF/ROE/SBC | 8,000-10,000 | `financials` 5年 + `verify-inputs` |
| 07 | `<管理层金句>`——X 的管理层值不值得托付 | 资本配置纪律 + 诚信检验 + 接班人 | 8,000-10,000 | `governance` + mx-search |
| 08 | 多少钱值得买，什么信号必须卖（系列终章） | DCF 三情景 + 红线清单 + 仓位框架 | 10,000-12,000 | `valuation` + `percentiles` + `financial_rigor.py three-scenario` |
...
Agent 路由失败可能间歇性发生；每次 TeamCreate/TaskCreate/后台 Agent 派发、等待或读取结果时都必须独立捕获，不得因前一次成功假设后续也成功；同一路由失败后不反复重试。若 TeamCreate、TaskCreate、后台 Agent 启动、等待或读取结果失败，Team Lead 不得把失败当作未发生。遇到 `model route not configured`、`timeout`、`permission denied` 或同类权限/路由/超时错误时，立即降级为「顺序角色模拟」，并在系列资料附录「Agent 降级记录」中记录：失败 Agent、错误原文摘要、降级方式、影响范围。错误原文摘要只保留必要信息，不写入密钥或本机隐私路径；错误摘要脱敏标准：可保留：模型名、错误码、HTTP 状态、非敏感错误类别，例如 `deepseek-v4-flash 路由未配置`；必须删除：本机绝对路径、token、cookie、账户名、API Key、完整请求头；影响范围需说明哪些审读角色由 Team Lead 模拟、哪些结论因此置信度下降。
...
Team Lead 先整理 8 篇文件路径、关键数字表、术语表和引用关系表；可派只读审读 Agent 分工扫描，也可由 Team Lead 顺序扫描。检查项：
1. 同一数字（市值、净利润、持股比例）跨篇是否一致
2. 同一术语（FBS、SBC、Non-IFRS）首次出现是否解释
3. 引用关系：02 篇说"详见 06 篇"是否真的对应
4. 要点回顾 vs 正文是否数字一致

审读 Agent 只输出问题清单和证据位置，不直接修改文件。所有修订由 Team Lead 完成。
