...
- **description**:
  0. **先执行 mx-search**（禁止跳过）：`python tools/lxr_data.py mx-search "{公司名} 近{N}天 公告 业绩预告 回购 增减持"`
...
- **description**:
  0. **先执行 mx-search**：`python tools/lxr_data.py mx-search "{行业} 监管 政策 罚款 整改 近{N}天"`
...
- **description**:
  0. **先执行 mx-search**：`python tools/lxr_data.py mx-search "{公司名} 竞争对手 行业 价格战 财报 近{N}天"`
...
- **description**:
  0. **先执行 mx-search**：`python tools/lxr_data.py mx-search "{公司名} 研报 评级 目标价 资金流向 近{N}天"`
...
**侦察方法**：
- **必须**先用 `python tools/lxr_data.py mx-search "..."`（自动 Windows 输出目录 + 1h 缓存）；失败时降级 WebSearch
