...
- **description**:
  ...
  7. **实时异动快照（mx-data）**：用 `mx-data` 取"{公司名} 最新价 涨跌幅 成交额 主力资金流向"实时快照，量化异动幅度与资金方向；**凡进入报告的 mx-data 快照数据点，必须标注 `_source: mx-data`**（含数值与时间）
...
**侦察方法**：
- **金融资讯优先走 `mx-search`**：公司事件/监管政策/行业动态/卖方点评等金融资讯查询，优先调用 `mx-search`（金融信源智能筛选），WebSearch 作为兜底（mx-search 限流或无结果时）
- 关键事件用 WebFetch 精读原始来源（公告原文、财报、监管文件）
- 对每个事件做"独立信源验证"——传言至少要 2 个独立来源
- **不要被标题党误导**：标题与正文不符的事件要标注"标题误导"

**数据来源标注规范（强制）**：
- 凡来自 `mx-search` 的事件/资讯，在该条事件后标注 `_source: mx-search`
- 凡来自 `mx-data` 的实时异动快照（价格/涨跌幅/资金流向等），标注 `_source: mx-data`
...
2. **完整事件时间线表格**（按日期倒序）：
   | 日期 | 事件 | 来源链接 | _source | 与股价异动相关性 | 持续性 |
   （`_source` 列填 `mx-search` / `mx-data` / `web`，与上述标注规范一致）
...
| 日期 | 维度 | 事件 | 来源 | _source | 异动归因权重 |
|------|------|------|------|---------|-----------|
| 2026-04-30 | 公司 | XX | 链接 | mx-search | 🔴 高 |
| 2026-04-29 | 行业 | XX | 链接 | mx-search | 🟡 中 |
| 2026-04-28 | 情绪 | XX | 链接 | mx-data | ⚪ 低 |

`_source` 列填 `mx-search` / `mx-data` / `web`，与各 Agent 侦察报告的来源标注一致；汇总表不得丢失该列。
...
9. **强制 `_source` 来源标注**——所有 `mx-search` 侦察结果标注 `_source: mx-search`，所有 `mx-data` 实时异动快照标注 `_source: mx-data`，WebSearch/WebFetch 结果标注 `_source: web`；最终归因报告的事件时间线必须保留 `_source` 列，便于读者追溯每条信息的获取渠道
