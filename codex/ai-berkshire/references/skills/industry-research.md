...
> ```
> 行业估值水位：`python tools/lxr_data.py industry-compare {行业内代表股}`（A股申万二级，`_source: lixinger`）。
> 产业动态资讯：`mx-search "{行业} 政策 最新"`（`--output-dir %TEMP%\mx_skills`，`_source: mx-search`）。
...
Team Lead 先建立行业资料包，全面搜索该行业所有上市公司；如平台支持后台 Agent，可把不同市场或产业链环节交给只读 Agent 复核和补漏，但不得让后台 Agent 自行联网或写文件。
...
只读 Agent 输出必须包含「使用的资料包条目」「疑似漏掉的市场/公司」「补数请求」。Team Lead 统一决定是否补搜并更新公司清单。
...
6. 最终将完整报告写入 `reports/{行业名}-industry-{YYYYMMDD}.md`，例如 `reports/核电-industry-20260629.md`
