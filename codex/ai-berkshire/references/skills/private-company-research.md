...
对 $ARGUMENTS 进行团队化深度研究分析。专为蚂蚁集团、小红书、SpaceX、Stripe 等未上市公司设计。Team Lead 统一搜索、取数、保存证据和写报告；6 个角色 Agent 只基于共享素材仓做独立拼图分析。后台 Agent 权限受限时，由 Team Lead 顺序模拟角色。
...
> **上市竞对/行业动态**（补充，非替代）：对标的的上市公司竞争对手，可用 `mx-search "{行业} {竞对} 融资 估值"`（**`_source: mx-search`**）与理杏仁 `industry-compare {上市竞对代码}`（**`_source: lixinger`**）作可比参照。未上市主体本身仍无理杏仁/mx-data 公司级财报。

### 第零步：权限安全素材仓（必须先建）

未上市公司研究可以多 Agent 分析，但不能让后台 Agent 自行联网取数。Team Lead 先建立 `reports/{公司名}/{公司名}-private-source-pack-{YYYYMMDD}.md` 或等价素材仓，至少包含：

1. 公司基础事实：成立时间、创始人、产品、融资轮次、投资方、估值口径。
2. 数据来源索引：每条来源的标题、链接、日期、来源类型、可信度。
3. 财务/用户/交易规模线索：所有数字必须标注 🟢/🟡/🔴 置信度。
4. 上市竞对数据：理杏仁或公开财报取数结果，用于可比公司锚定。
5. 负面和反证材料：诉讼、监管、裁员、投诉、竞争受挫、估值下调。
6. 已知黑区：确认找不到或无法验证的信息，不用推测填满。

角色 Agent 只能分析素材仓。若发现缺关键材料，输出「补数请求」交给 Team Lead；Team Lead 补充素材仓后再进入下一轮分析。

### 第一步：展示团队框架

向用户展示以下团队结构，然后进入素材仓分派：
...
Agent 路由失败可能间歇性发生；每次 TeamCreate/TaskCreate/后台 Agent 派发、等待或读取结果时都必须独立捕获，不得因前一次成功假设后续也成功；同一路由失败后不反复重试。若 TeamCreate、TaskCreate、后台 Agent 启动、等待或读取结果失败，Team Lead 不得把失败当作未发生。遇到 `model route not configured`、`timeout`、`permission denied` 或同类权限/路由/超时错误时，立即降级为「顺序角色模拟」，并在最终报告或素材仓附录「Agent 降级记录」中记录：失败 Agent、错误原文摘要、降级方式、影响范围。错误原文摘要只保留必要信息，不写入密钥或本机隐私路径；错误摘要脱敏标准：可保留：模型名、错误码、HTTP 状态、非敏感错误类别，例如 `deepseek-v4-flash 路由未配置`；必须删除：本机绝对路径、token、cookie、账户名、API Key、完整请求头；影响范围需说明哪些角色由 Team Lead 模拟、哪些结论因此置信度下降。
...
如果当前平台支持后台 Agent 且不会触发权限阻塞，启动6个只读分析 Agent；能并行就并行，不能并行就顺序执行。不要让后台 Agent 自行联网、运行命令或写文件。
...
**研究方法**：
1. 只使用 Team Lead 提供的素材仓、来源索引、摘录和工具输出，不调用 WebSearch/WebFetch/Bash/Write/Edit
2. 如果某个维度需要新增搜索，用「补数请求」列出关键词、目标来源和为什么需要该资料
3. Team Lead 建素材仓时优先覆盖以下信息源：
   - 高可信度：招股书、监管文件、上市公司年报中的关联披露
   - 中可信度：晚点LatePost、The Information、36氪、Bloomberg、Reuters、TechCrunch
   - 辅助验证：知乎、脉脉、Glassdoor、天眼查、企查查
4. 重要数据只解读已完成的交叉验证结果；若素材仓缺少第二来源，标注为「待核验」
5. 对所有推算给出假设链和置信度，不把推算写成事实
...
- 等待全部6份报告到齐；如有补数请求，Team Lead 统一补素材仓后再补一轮只读分析
...
1. **权限安全优先**——6 个角色可以并行只读分析，但搜索、取数、命令执行、文件写入只由 Team Lead 负责
...
7. **允许顺序角色模拟**——后台 Agent 权限受限时，不要阻塞流程；Team Lead 按 6 个角色顺序完成同等分析
8. **中英文搜索由 Team Lead 完成**——未上市公司信息可能分布在中英文媒体，需要两种语言搜索并写入素材仓
