...
`weight` 可用百分数或小数，工具会自动归一化。`expected_return` 为预期年化收益，`conviction` 为确定性/置信度，均可用小数或百分数。报告中必须引用 `portfolio_analyzer` 输出的 `overall_health`、`concentration`、`exposures`、`risk_flags`、`correlation_risks`、`stress_tests`、`opportunity_cost` 与 `rebalance_suggestions`。该工具只做组合结构诊断和机械再平衡提示，不替代后续个股估值、新闻核验和最终调仓判断。
...
优先引用 `portfolio_analyzer` 的 `opportunity_cost`，按 `expected_return × conviction` 计算风险调整后预期年化，并与现金门槛（默认 4%）比较；缺少输入的持仓必须标注“数据不足”，不得用主观排序替代。
