...
执行前确认研究深度：`--depth lite`（5-10 分钟快速评估，仅产出研究底稿不做发布）、`--depth standard`（完整三阶段流程，默认）或 `--depth deep`（机构级深度，附加同行分析 Agent）。lite 模式报告目录加 `-lite` 后缀（如 `reports/腾讯-earnings-2025Q4-lite/`）。本技能三档行为差异详见 `docs/depth-profiles.md`「earnings-team」小节。
