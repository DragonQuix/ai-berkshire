...
执行前确认研究深度：`--depth lite`（5-10 分钟快速评估）、`--depth standard`（完整研究，默认）或 `--depth deep`（机构级深度）。lite 模式报告文件名加 `-lite` 后缀（如 `AI算力-funnel-lite-20260509.md`）。本技能三档行为差异详见 `docs/depth-profiles.md`「industry-funnel」小节。
